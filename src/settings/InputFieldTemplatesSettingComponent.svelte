<script lang="ts">
	...
	import {Button, ModalButtonGroup, TextInput} from 'obsidian-svelte';
	...
	<table>
		...
		<tbody>
			{#each inputFieldTemplates as template}
				<InputFieldTemplateSettingComponent template={template} on:delete-template={(evt) => deleteTemplate(evt.detail.name)}></InputFieldTemplateSettingComponent>
			{/each}
		</tbody>
	</table>

	<Button on:click={() => addTemplate()} variant="primary" tolltip="Create New Template">Add Template</Button>

	{#if errorCollection}
		<div>
			<h3 class="mod-error">Some Templates Failed to Parse</h3>

			<ErrorCollectionComponent errorCollection={errorCollection}></ErrorCollectionComponent>
		</div>
	{/if}


	<ModalButtonGroup>
		<Button on:click={() => save()} variant="primary" tolltip="Save Changes">Save</Button>
		<Button on:click={() => cancel()} tolltip="Revert Changes">Cancel</Button>
	</ModalButtonGroup>
</div>
